...
async function packageLockCommand(uri, options = {}) {
  const packageLock = await retrieveFile(uri); // 首先根据uri读取package-lock文件
...
async function packageCommand(name, version, options = {}) {
  const crawler = new Crawler(); // 存储缓存信息
...
async function packageCountCommand(name, versionNumber, options = {}) {
	const crawler = new Crawler(); // 存储缓存信息
	const tarballsSet = await crawler.getDependenciesFromVersionNumber({
		name, versionNumber, options
	})
	console.log('tarballsSet:', tarballsSet)
	// await downloader.downloadFromIterable(tarballsSet, options, crawler.packagesCache);
}
